...
import { CATEGORIES, SUB_SKILLS_MAP } from '../../features/activities/data/activitiesData';
import { METRO_LINES, METRO_STATIONS } from '../../features/metro/data/metroData';
  ...
  const renderFilterForm = () => (
    <div className="space-y-6 text-slate-900 text-xs">
      ...
      <div className="space-y-2">
        <label htmlFor="sidebar-category-select" className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">
          Category & Sub-skills
        </label>
        <div className="relative">
          <select
            id="sidebar-category-select"
            aria-label="Category & Sub-skills"
      ...
      <div className="space-y-2 pt-4 border-t border-slate-100">
        <div className="flex items-center justify-between">
          <label htmlFor="sidebar-metro-select" className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">
            Metro Station
          </label>
          {filters.metroStationIds.length > 0 && (
        ...
        <div className="relative flex items-center">
          <select
            ...
            className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-3 pr-8 py-2.5 min-h-[44px] leading-normal text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900/10 transition-all cursor-pointer appearance-none"
          >
            <option value="all">All Metro Lines ↓</option>
            {METRO_LINES.map((line) => (
              <option key={line.id} value={line.id} className="py-1">
                {line.name}
              </option>
            ))}
          </select>
        ...
        <div className="max-h-48 overflow-y-auto pr-2 pb-1 space-y-1 p-2 rounded-2xl bg-slate-50 border border-slate-200/60 scrollbar-thin scrollbar-thumb-neutral-200">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {(['All', 'Adults', 'Children', 'Corporate'] as AudienceType[]).map((aud) => {
            ...
            return (
              ...
              >
                {aud === 'All' ? 'All Audiences' : aud === 'Children' ? 'Kids' : aud}
              </button>
            );
          })}
        </div>
      ...
      <div className="space-y-2.5 pt-4 border-t border-slate-100">
        <div className="flex items-center justify-between">
          <label htmlFor="sidebar-price-range" className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">
            Max Price
          </label>
          ...
          {showAdvanced && (
            ...
            >
              ...
              <div className="space-y-1.5">
                <label htmlFor="sidebar-goal-select" className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
                  Discovery Goal
                </label>
                <div className="relative">
                  <select
                    id="sidebar-goal-select"
