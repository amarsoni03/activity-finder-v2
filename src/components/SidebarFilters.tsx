...
import { Category, RegularityType, TimeOfDay, SkillLevel, FilterState, DayOfWeek, GoalType, AudienceType, DeliveryFilter, LanguageFilter } from '../types';
...
export const SidebarFilters: React.FC<SidebarFiltersProps> = ({
  ...
  const currentLineStations = filters.metroLineId === 'all'
    ? METRO_STATIONS
  ...
  const PRICE_TIERS = [
    ...
    { label: 'Mid-range', val: 7000 },
    { label: 'Max', val: 15000 },
  ];

  // Render main filter form content
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-2.5">
          ...
          <div className="relative">
            <select
              ...
              className="w-full appearance-none bg-slate-50 border border-slate-200/80 rounded-2xl px-4 py-3 text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-400 transition-all cursor-pointer"
            >
              {CATEGORIES.map((cat) => (
            ...
            <div className="pt-1 space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                <button
                  ...
                  className={`px-3 py-1.5 text-xs rounded-xl font-medium transition-all cursor-pointer min-h-[34px] ${
        ...
        <div className="space-y-2.5 pt-4 border-t border-slate-100">
          ...
          <div className="max-h-40 overflow-y-auto space-y-1 p-2.5 rounded-2xl bg-slate-50/80 border border-slate-200/70 scrollbar-thin">
            {currentLineStations.map((st) => {
              const isChecked = filters.metroStationIds.includes(st.id);
              return (
                <label
                  key={st.id}
                  className="flex items-center space-x-2 text-xs text-slate-700 cursor-pointer p-1 rounded-xl hover:bg-white transition-colors"
                >
                  <input
                    type="checkbox"
                    checked={isChecked}
        ...
        <div className="space-y-2.5 pt-4 border-t border-slate-100">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            <button
              type="button"
              onClick={toggleToday}
              className={`py-2 px-2 text-xs font-semibold rounded-xl text-center transition-all cursor-pointer min-h-[38px] ${
                isTodaySelected
              ...
              {ALL_DAYS.map((day) => {
                ...
                return (
                  <button
                    ...
                    className={`py-2 text-[11px] font-bold rounded-xl text-center transition-all cursor-pointer min-h-[34px] ${
                      isSelected
            ...
            {(['All', 'Adults', 'Children', 'Corporate'] as AudienceType[]).map((aud) => {
              ...
              return (
                <button
                  ...
                  className={`py-2 px-3 text-xs font-semibold rounded-xl text-center transition-all cursor-pointer min-h-[38px] ${
                    isSelected
                      ? 'bg-slate-900 text-white shadow-xs'
                      : 'bg-slate-50 text-slate-700 hover:bg-slate-100 border border-slate-200/60'
                  }`}
      ...
      {/* 2. SECONDARY FILTERS                                     */}
      {/* ======================================================== */}
      <div className="space-y-5 pt-5 border-t-2 border-slate-100">
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {[0, 4.0, 4.5, 4.8].map((stars) => (
              <button
                ...
                className={`py-2 text-xs font-bold rounded-xl transition-all cursor-pointer min-h-[36px] ${
        ...
        <div className="space-y-2.5 pt-4 border-t border-slate-100">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {[
              ...
              { id: 'English & Russian', label: '🇬🇧🇷🇺 English & Russian' },
            ].map((langOption) => {
              ...
              return (
                <button
                  key={langOption.id}
                  type="button"
                  onClick={() => onFilterChange({ language: langOption.id as LanguageFilter })}
                  className={`py-2 px-2 text-[11px] font-bold rounded-xl text-center transition-all cursor-pointer ${
                    isActive
                      ? 'bg-slate-900 text-white shadow-xs'
                      : 'bg-slate-50 text-slate-700 hover:bg-slate-100 border border-slate-200/60'
                  }`}
        ...
        {showAdvanced && (
          <div className="space-y-4 pt-3 transition-all">
            <div className="space-y-2">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none bg-slate-50 border border-slate-200/80 rounded-2xl px-4 py-3 text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900/10 transition-all cursor-pointer"
                >
            ...
            <div className="space-y-2 pt-2 border-t border-slate-100">
              ...
              <label className="flex items-center space-x-2 text-xs font-medium text-slate-700 cursor-pointer p-1.5 rounded-xl hover:bg-slate-50 transition-colors">
                <input
                  type="checkbox"
    ...
    <>
      ...
      <div className="lg:hidden mb-4">
        <button
          ...
          className="w-full bg-white border border-slate-200/80 rounded-2xl px-5 py-3.5 flex items-center justify-between text-slate-900 shadow-2xs hover:bg-slate-50 transition-all cursor-pointer"
        >
          <div className="flex items-center space-x-2.5">
            <Filter className="w-4 h-4 text-slate-600" />
            <span className="text-xs font-bold uppercase tracking-wider text-slate-700">Filter Activities</span>
            {isFiltered && (
          ...
          <span className="text-xs font-bold text-slate-900 bg-slate-100 px-3 py-1 rounded-full">
            {resultsCount} Activities
          </span>
        </button>
      </div>

      {/* Desktop Sticky Sidebar */}
      <aside className="hidden lg:block bg-white rounded-3xl p-6 border border-slate-200/70 shadow-2xs text-slate-900 space-y-6">
        {/* Header */}
        <div className="flex items-center justify-between pb-4 border-b border-slate-100">
          ...
          {isFiltered && (
            <button
              ...
              className="text-[11px] font-semibold text-slate-400 hover:text-slate-900 flex items-center space-x-1 transition-colors cursor-pointer"
      ...
      {/* Mobile Drawer */}
      {mobileDrawerOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex flex-col bg-white overflow-hidden animate-fade-in">
          {/* Header */}
          ...
          {/* Sticky Footer */}
          <div className="sticky bottom-0 z-10 bg-white border-t border-slate-100 p-4 shadow-lg flex items-center gap-3">
            {isFiltered && (
              <button
                onClick={onResetFilters}
                className="px-4 py-3.5 border border-slate-200 rounded-2xl text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all cursor-pointer"
              >
                Reset
              </button>
            )}
            <button
              onClick={() => setMobileDrawerOpen(false)}
              className="flex-1 bg-slate-900 text-white rounded-2xl py-3.5 text-xs font-bold shadow-md hover:bg-slate-800 transition-all cursor-pointer text-center"
            >
