      ...
      <div>
        ...
        <div className="relative h-44 sm:h-56 w-full rounded-xl sm:rounded-2xl overflow-hidden mb-3 sm:mb-4 bg-slate-100 shrink-0">
          ...
          <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
            <span className="px-3 py-1 text-[11px] font-bold tracking-wide rounded-full bg-white/95 backdrop-blur-md text-slate-900 shadow-2xs border border-white/40">
              {activity.category}
            </span>
        ...
        <div className="flex items-center space-x-2 mb-1.5">
          ...
          <span className="text-[10px] font-bold text-slate-700 bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200/80">
            {activity.deliveryMode === 'Live Online' ? '💻 Live Online' : activity.deliveryMode === 'Self-Paced' ? '🎬 Self-Paced' : activity.deliveryMode === 'Hybrid' ? '🔄 Hybrid' : '📍 In Person'}
          </span>
        </div>
        ...
        <div className="space-y-1.5 text-xs text-slate-700 mb-3 bg-slate-50/70 p-3 rounded-2xl border border-slate-100">
          ...
          <div className="flex items-start gap-2 text-slate-600">
            ...
            <span className="min-w-0 break-words">
              ...
              <span className="text-slate-400 font-normal"> ({walkMins}m walk)</span>
            </span>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-slate-500 mb-3 font-medium">
          ...
          <div className="flex items-center gap-1 font-bold text-slate-800 shrink-0">
            ...
            <span className="text-slate-400 font-normal">({activity.reviewCount})</span>
          </div>
          {activity.duration && (
        ...
        {/* Short Description */}
        <p className="text-xs text-slate-500 line-clamp-2 mb-4 leading-relaxed font-normal sm:min-h-[36px]">
          {activity.shortDescription}
        </p>
      </div>

      {/* Footer: Quiet Provider Info + Micro Trust Badge + Single Primary CTA */}
      <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between text-xs mt-auto">
        <div className="text-slate-500 text-[11px] font-medium flex items-center gap-1.5 min-w-0 sm:pr-2 sm:max-w-[55%]">
          <span className="truncate">Taught by {activity.instructorName || activity.teacher?.name || 'Instructor'}</span>
          <ProviderTrustBadge
            variant="micro"
