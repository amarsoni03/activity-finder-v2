...
import { STORAGE_KEYS } from '../services/activityStorage';
...
export const WaitlistModal: React.FC<WaitlistModalProps> = ({
  ...
  const [waitlistState, setWaitlistState] = useState<WaitlistState>(() => {
    const local = localStorage.getItem(STORAGE_KEYS.USER_WAITLIST_STATE);
    if (local) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem(STORAGE_KEYS.USER_WAITLIST_STATE, JSON.stringify(waitlistState));
  }, [waitlistState]);
